...
export const DocumentTypeSelector = ({ value, error, onChange, ...otherProps }: Props) => {
    return (
        <Select
            value={value?.value || ""}
            error={!!error}
            onChange={(e) => onChange?.(e.target.value as DocumentTypesValues)}
            {...otherProps}
        >
